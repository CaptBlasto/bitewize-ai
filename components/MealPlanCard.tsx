...
export default function MealPlanCard({ userPlan, selectedDate, onPlanGenerated, onMealPlanChange }: Props) {
  ...
  async function exportToPDF() {
    if (!mealPlan || !exportRef.current) return;
    setExporting(true);

    // Wait for all accordions + grocery list to fully expand (CSS transition: 300ms)
    ...
    try {
      ...
      const canvas = await html2canvas(exportRef.current, {
        scale: 2,
        useCORS: true,
        backgroundColor: "#111113", // bw-card
        logging: false,
      });
      ...
      pdf.text("Bitewize AI \u2014 Meal Plan", margin, margin + 5);

      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(9);
      pdf.setTextColor(161, 161, 170);
      pdf.text(displayDate, margin, margin + 12);

      const headerH = margin + 18; // top of image on first page

      // ── Multi-page image slicing ──────────────────────────────────────────
      const pixelsPerMm = canvas.width / contentW;
      const totalImgHeightMm = canvas.height / pixelsPerMm;
      const footerH = 12; // reserved at bottom of every page

      let yMm = 0;  // how many mm of the image we've placed so far
      let pageNum = 0;

      while (yMm < totalImgHeightMm) {
        const topY = pageNum === 0 ? headerH : margin;
        const availH = pageH - topY - footerH;
        const sliceHeightMm = Math.min(totalImgHeightMm - yMm, availH);
        const sliceHeightPx = Math.ceil(sliceHeightMm * pixelsPerMm);
        const sliceYPx = Math.ceil(yMm * pixelsPerMm);
        ...
        pdf.addImage(slice.toDataURL("image/png"), "PNG", margin, topY, contentW, sliceHeightMm);

        // Footer on every page
        pdf.setFont("helvetica", "normal");
        pdf.setFontSize(7);
        pdf.setTextColor(161, 161, 170);
        pdf.text("Generated by Bitewize AI \u00B7 bitewizeai.com", margin, footerY);
          ...
          {mealPlan.macros.grocery_list?.length > 0 && (
            <div className="mt-4">
              ...
              <div
                ...
                style={{ maxHeight: (groceryOpen || exporting) ? "9999px" : "0" }}
              >
...
function MealAccordion({
  meal, index, isOpen, forceOpen, onToggle,
}: {
  meal: Meal; index: number; isOpen: boolean; forceOpen?: boolean; onToggle: () => void;
}) {
